...
export function ModuleConfiguration({
  ...
  const activeModuleIds = useMemo(
    () => new Set(activeModules.map((module) => module.moduleId)),
    [activeModules]
  );

  const visibleModules = useMemo(
    () => appModules.filter((module) => !showOnlyActive || activeModuleIds.has(module.id)),
    [appModules, showOnlyActive, activeModuleIds]
  );
      ...
      <div className="p-6 pb-4 border-b border-slate-100 bg-slate-50/50">
        ...
        {readOnly ? (
          <p className="mt-3 text-xs text-slate-500">
            {t("module_config_read_only") ||
              "Module activation is managed by the platform administrator; tenant admins can only view active modules."}
          </p>
        ) : null}
      </div>
      ...
      <div className="divide-y divide-slate-100">
        {visibleModules.length === 0 ? (
          <div className="p-5 text-sm text-slate-500">
            {showOnlyActive
              ? t("no_active_modules") || "No active modules are currently assigned to this tenant."
              : t("no_modules_available") || "No modules available."}
          </div>
        ) : (
              ...
              >
                <div className="pt-0.5">
                  <button
                    type="button"
                    onClick={() => {
                      if (readOnly || !handleModuleToggle) return;
                      handleModuleToggle(mod.id, mod.required);
                    }}
                    disabled={readOnly || mod.required}
                    className={cn(
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <h3 className={cn("text-sm font-bold", isActive ? "text-slate-900" : "text-slate-500")}>
                      {mod.name}
                      {mod.required && (
                        <span className="ml-2 text-[9px] uppercase tracking-wider bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-semibold border border-slate-200">
                          {t("module_required")}
                        </span>
                      )}
                      {isActive && !mod.required && (
                        <span className="ml-2 text-[9px] uppercase tracking-wider bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded font-semibold border border-blue-200">
                          {t("module_active")}
                        </span>
                      )}
                    </h3>
