...
export function RejectExamDialog({
  ...
  const handleConfirm = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/v1/laboratory/${exam.id}/reject`, {
        method: "PATCH",
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("reject_results")}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          ...
          <p className="text-xs text-slate-500">{t("reject_results_desc")}</p>
