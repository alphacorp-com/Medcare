                  ...
                  {filteredInventory.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-right space-x-2">
                        <button className="text-blue-600 hover:underline font-medium" onClick={() => { setRestockMed(item); setRestockAmount(0); setRestockOpen(true); }}>{t('restock')}</button>
                        <button className="text-slate-500 hover:text-slate-800 hover:underline font-medium" onClick={() => { setEditMedForm(item); setEditMedOpen(true); }}>{tc('edit')}</button>
                      </td>
                    </tr>
                  ))}
        ...
        <SheetContent className="sm:max-w-md w-full right-0 p-0 flex flex-col bg-slate-50">
          ...
          <SheetFooter className="p-4 border-t border-slate-200 bg-white shrink-0">
            ...
            <Button className="text-xs h-8 bg-blue-600 hover:bg-blue-700" disabled={savingMed || !medForm.name} onClick={async () => {
              ...
              {savingMed ? t('saving') : tc('save')}
            </Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
      <Sheet open={editMedOpen} onOpenChange={setEditMedOpen}>
        <SheetContent className="sm:max-w-md w-full right-0 p-0 flex flex-col bg-slate-50">
          <SheetHeader className="p-4 border-b border-slate-200 bg-white shrink-0">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => setEditMedForm({ ...editMedForm, stock: parseInt(e.target.value) || 0 })}
                      className="h-8 text-xs bg-white border-slate-200"
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input
                      type="number"
                      value={editMedForm.threshold}
                      onChange={(e) => setEditMedForm({ ...editMedForm, threshold: parseInt(e.target.value) || 0 })}
                      className="h-8 text-xs bg-white border-slate-200"
                    />
                  </div>
                </div>
          ...
          <SheetFooter className="p-4 border-t border-slate-200 bg-white shrink-0">
            <Button variant="outline" className="text-xs h-8" onClick={() => setEditMedOpen(false)} disabled={savingEditMed}>{tc('cancel')}</Button>
            <Button className="text-xs h-8 bg-blue-600 hover:bg-blue-700" disabled={savingEditMed || !editMedForm?.name} onClick={async () => {
              ...
              {savingEditMed ? t('saving') : tc('save')}
            </Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
      ...
      <Sheet open={restockOpen} onOpenChange={setRestockOpen}>
        <SheetContent className="sm:max-w-sm w-full right-0 p-0 flex flex-col bg-slate-50">
          <SheetHeader className="p-4 border-b border-slate-200 bg-white shrink-0">
            ...
            <SheetDescription className="text-xs">
              {t('restock_desc')} {restockMed?.name}.
            </SheetDescription>
          </SheetHeader>
          <div className="p-4 flex-1 overflow-y-auto space-y-4">
            <div className="bg-slate-100 p-3 rounded border border-slate-200 text-xs flex justify-between">
              ...
              <span className="font-mono font-bold text-slate-900">{restockMed?.stock} {restockMed?.unit}</span>
            </div>
            <div className="space-y-1">
            ...
            {restockAmount > 0 && restockMed && (
               <div className="bg-green-50 p-3 rounded border border-green-200 text-xs flex justify-between">
                 ...
                 <span className="font-mono font-bold text-green-800">{(restockMed.stock) + restockAmount} {restockMed.unit}</span>
               </div>
            )}
          </div>
          <SheetFooter className="p-4 border-t border-slate-200 bg-white shrink-0">
            <Button variant="outline" className="text-xs h-8" onClick={() => setRestockOpen(false)} disabled={savingRestock}>{tc('cancel')}</Button>
            <Button className="text-xs h-8 bg-blue-600 hover:bg-blue-700" disabled={savingRestock || restockAmount <= 0} onClick={async () => {
              ...
              {savingRestock ? t('saving') : tc('save')}
            </Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
