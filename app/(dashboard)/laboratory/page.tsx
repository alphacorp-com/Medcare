    ...
    return (
      <div className="flex h-full items-center justify-center">
         <div className="text-center p-8 bg-white border border-slate-200 rounded-lg max-w-md shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-500">The Laboratory module is not active for your role, or has been disabled across the environment.</p>
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      <div className="flex items-center justify-between shrink-0 bg-white p-4 rounded border border-slate-200 shadow-sm">
        <div>
          <h1 className="text-lg font-bold text-slate-800">Laboratory Dashboard</h1>
          <p className="text-xs text-slate-500 mt-1">Manage exam requests, analyses, and results validation.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-4 shrink-0">
        <div className="bg-white p-4 rounded border border-slate-200 shadow-sm flex items-end justify-between hover:border-blue-300 transition-colors cursor-pointer" onClick={() => setFilter("Pending Sample")}>
          <div>
            <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">To Collect</div>
        ...
        <div className="bg-white p-4 rounded border border-slate-200 shadow-sm flex items-end justify-between hover:border-blue-300 transition-colors cursor-pointer" onClick={() => setFilter("Awaiting Validation")}>
          <div>
            <div className="text-xs font-semibold text-yellow-600 uppercase tracking-wider mb-1">To Validate</div>
      ...
      <div className="flex-1 flex flex-col bg-white rounded border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-2 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
           <div className="relative w-96 flex-1">
              ...
              <Input
                type="search"
                placeholder="Search exam ID, patient..."
                value={search}
            ...
            <div className="flex bg-slate-200/50 p-1 rounded-md ml-4">
              ...
              <button onClick={() => setFilter("Pending Sample")} className={cn("px-3 py-1 rounded text-[10px] uppercase font-bold", filter === "Pending Sample" ? "bg-white shadow-sm text-slate-700" : "text-slate-500 hover:text-slate-700")}>To Collect</button>
              ...
              <button onClick={() => setFilter("Awaiting Validation")} className={cn("px-3 py-1 rounded text-[10px] uppercase font-bold", filter === "Awaiting Validation" ? "bg-white shadow-sm text-slate-700" : "text-slate-500 hover:text-slate-700")}>Validation</button>
            </div>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="text-xs divide-y divide-slate-100">
                  ...
                  <td className="px-4 py-2 text-right">
                    ...
                    >
                      {ex.status === 'Pending Sample' ? 'Collect' : 
                       ...
                       ex.status === 'Awaiting Validation' ? 'Validate' : 'View'}
                    </button>
                  </td>
                </tr>
              ))}
              {filteredExams.length === 0 && (
                <tr>
                   <td colSpan={7} className="text-center py-8 text-slate-500 text-xs">No exams match your criteria.</td>
                </tr>
              )}
            </tbody>
          </table>
