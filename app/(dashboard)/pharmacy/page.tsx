    ...
    return (
      <div className="flex h-full items-center justify-center">
         <div className="text-center p-8 bg-white border border-slate-200 rounded-lg max-w-md shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-500">The Pharmacy & Circuit Médicament module is not active for your role, or has been disabled across the environment.</p>
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      <div className="flex items-center justify-between shrink-0 bg-white p-4 rounded border border-slate-200 shadow-sm">
        <div>
          <h1 className="text-lg font-bold text-slate-800">Pharmacy & Stock</h1>
          <p className="text-xs text-slate-500 mt-1">Manage prescriptions (Circuit Médicament) and inventory.</p>
        </div>
      </div>
      ...
      <Tabs defaultValue="queue" className="flex-1 flex flex-col gap-4 overflow-hidden">
        <TabsList className="bg-transparent p-0 flex justify-start gap-6 border-b border-slate-200 rounded-none h-10 w-full shrink-0 px-2">
          ...
          >
            <PillBottle className="h-4 w-4 mr-2" /> Prescriptions Queue
          </TabsTrigger>
          <TabsTrigger 
            value="inventory" 
            className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-blue-600 rounded-none data-[state=active]:text-blue-700 h-full text-sm font-semibold tracking-wide"
          >
            <PackageSearch className="h-4 w-4 mr-2" /> Stock & Inventory
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="queue" className="m-0 border-none outline-none flex-1 flex flex-col overflow-hidden gap-4">
          <div className="grid gap-4 md:grid-cols-3 shrink-0">
            <div className="bg-white p-4 rounded border border-slate-200 shadow-sm flex items-end justify-between hover:border-blue-300 transition-colors cursor-pointer" onClick={() => setFilter("Pending Queue")}>
              <div>
                <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">To Validate</div>
            ...
            <div className="bg-white p-4 rounded border border-slate-200 shadow-sm flex items-end justify-between hover:border-blue-300 transition-colors cursor-pointer" onClick={() => setFilter("Validated")}>
              <div>
                <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">To Dispense</div>
            ...
            <div className="bg-red-50 p-4 rounded border border-red-200 shadow-sm flex items-end justify-between">
              <div>
                 <div className="text-xs font-semibold text-red-800 uppercase tracking-wider mb-1">Interactions Alerts</div>
          ...
          <div className="flex-1 flex flex-col bg-white rounded border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-2 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
               <div className="relative w-96 flex-1">
                  ...
                  <Input
                    type="search"
                    placeholder="Search prescription ID, patient..."
                    value={search}
                ...
                <div className="flex bg-slate-200/50 p-1 rounded-md ml-4">
                  <button onClick={() => setFilter("All")} className={cn("px-3 py-1 rounded text-[10px] uppercase font-bold", filter === "All" ? "bg-white shadow-sm text-slate-700" : "text-slate-500 hover:text-slate-700")}>All</button>
                  <button onClick={() => setFilter("Pending Queue")} className={cn("px-3 py-1 rounded text-[10px] uppercase font-bold", filter === "Pending Queue" ? "bg-white shadow-sm text-slate-700" : "text-slate-500 hover:text-slate-700")}>Pending Validation</button>
                  <button onClick={() => setFilter("Validated")} className={cn("px-3 py-1 rounded text-[10px] uppercase font-bold", filter === "Validated" ? "bg-white shadow-sm text-slate-700" : "text-slate-500 hover:text-slate-700")}>To Dispense</button>
                </div>
            </div>
                ...
                <tbody className="text-xs divide-y divide-slate-100">
                  {filteredPrescriptions.map((rx) => (
                    <tr key={rx.id} className="hover:bg-blue-50/50 cursor-pointer" onClick={() => setSelectedRx(rx)}>
                      ...
                      <td className="px-4 py-2">{rx.items} medications</td>
                      ...
                      <td className="px-4 py-2 text-right">
                        ...
                        >
                          {rx.status === 'Pending Queue' ? 'Validate' : rx.status === 'Validated' ? 'Dispense' : 'View'}
                        </button>
                      </td>
                    </tr>
                  ))}
                  {filteredPrescriptions.length === 0 && (
                    <tr>
                       <td colSpan={7} className="text-center py-8 text-slate-500 text-xs">No prescriptions match your criteria.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <TabsContent value="inventory" className="m-0 border-none outline-none flex-1 flex flex-col overflow-hidden gap-4">
          <div className="grid gap-4 md:grid-cols-4 shrink-0">
            <div className="bg-white p-4 rounded border border-slate-200 shadow-sm">
               <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Total SKUs</div>
            ...
            <div className="bg-yellow-50 p-4 rounded border border-yellow-200 shadow-sm flex items-end justify-between">
              <div>
                 <div className="text-xs font-semibold text-yellow-800 uppercase tracking-wider mb-1">Low Stock Alerts</div>
                 <div className="text-3xl font-bold text-yellow-700">{inventory.filter(i => i.status === 'Low Stock').length}</div>
              </div>
              <BatteryWarning className="h-8 w-8 text-yellow-300" />
            </div>
            <div className="bg-red-50 p-4 rounded border border-red-200 shadow-sm flex items-end justify-between">
              <div>
                 <div className="text-xs font-semibold text-red-800 uppercase tracking-wider mb-1">Out of Stock</div>
                 <div className="text-3xl font-bold text-red-700">{inventory.filter(i => i.status === 'Out of Stock').length}</div>
              </div>
          ...
          <div className="flex-1 flex flex-col bg-white rounded border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-2 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
               <div className="relative w-96 flex-1">
                  ...
                  <Input
                    type="search"
                    placeholder="Search medication name, ID, category..."
                    value={invSearch}
                ...
                <div className="flex gap-2 ml-4">
                  ...
                  <Button size="sm" className="bg-blue-600 hover:bg-blue-700 h-8 text-xs text-white">
                    <Plus className="mr-2 h-3 w-3" /> Add Medication
                  </Button>
                </div>
            </div>
            <div className="flex-1 overflow-auto">
                  ...
                  {filteredInventory.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-right space-x-2">
                        <button className="text-blue-600 hover:underline font-medium">Restock</button>
                        <button className="text-slate-500 hover:text-slate-800 hover:underline font-medium">Edit</button>
                      </td>
                    </tr>
                  ))}
                  {filteredInventory.length === 0 && (
                    <tr>
                       <td colSpan={7} className="text-center py-8 text-slate-500 text-xs">No medications found in inventory.</td>
                    </tr>
                  )}
                </tbody>
              </table>
               ...
               <div className="flex items-start justify-between">
                 <div>
                   ...
                   <SheetDescription className="text-xs mt-1">
                     Ordered by {selectedRx.prescriber} on {format(selectedRx.date, "PPP 'at' p")}
                   </SheetDescription>
                 </div>
                 {selectedRx.alert && (
                ...
                <div className="bg-white p-3 rounded border border-slate-200 shadow-sm flex items-center justify-between">
                   <div className="flex items-center gap-3">
                     ...
                     <div>
                       ...
                       <div className="text-[10px] font-mono text-slate-500">IPP: {selectedRx.ipp}</div>
                     </div>
                   </div>
                   <Button variant="outline" size="sm" className="text-xs h-7">View Full Chart</Button>
                </div>
                ...
                {selectedRx.alert && (
                  <div className="bg-red-50 p-3 rounded border border-red-200 shadow-sm flex flex-col gap-2">
                    ...
                    <p className="text-xs text-red-700 font-medium leading-relaxed">
                       Potential Major interaction between prescribed [Medication A] and patient's active [Medication B]. Risk of increased hypotensive effect. Clinical evaluation recommended before dispensing.
                    </p>
                  </div>
                )}
                         ...
                         <div key={i} className="bg-white p-3 rounded border border-slate-200 shadow-sm">
                            <div className="flex justify-between items-start">
                               <div>
                                  ...
                                  <div className="text-[11px] text-slate-600 mt-0.5">Take 1 capsule by mouth every 8 hours for 7 days.</div>
                               </div>
                 ...
                 <div className="flex gap-2">
                    {selectedRx.status === 'Pending Queue' && (
                       <>
                         ...
                         <Button className="text-xs h-8 bg-blue-600 hover:bg-blue-700" onClick={() => { setSelectedRx({...selectedRx, status: 'Validated'}); }}>
                           <Check className="mr-2 h-3.5 w-3.5" /> Validate Order
                         </Button>
                       </>
                    )}
                    {selectedRx.status === 'Validated' && (
                       <Button className="text-xs h-8 bg-green-600 hover:bg-green-700 text-white" onClick={() => { setSelectedRx({...selectedRx, status: 'Dispensed'}); }}>
                         <PillBottle className="mr-2 h-3.5 w-3.5" /> Process & Dispense
                       </Button>
                    )}
                    {selectedRx.status === 'Dispensed' && (
                       <span className="text-xs font-bold uppercase tracking-wider text-green-700 flex items-center px-4 py-1.5 bg-green-100 rounded">
                          <Check className="mr-2 h-4 w-4" /> Fulfilled
                       </span>
                    )}
                 </div>
             </SheetFooter>
