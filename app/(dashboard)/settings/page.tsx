...
const APP_MODULES = [
  { id: "MODULE_CORE_PATIENT", name: "Patient Management", desc: "Core demographic tracking, centralized medical records, and quick patient search.", required: true },
  { id: "MODULE_ADMISSION", name: "Admissions & Bed Flow", desc: "Manage inpatient stays, room/bed allocations, and discharge planning." },
  { id: "MODULE_PHARMACY", name: "Pharmacy & Inventory", desc: "Track medication stock, process prescriptions, and handle clinical dispensations." },
  { id: "MODULE_LAB", name: "Laboratory System", desc: "Request exams, track sample arrays, and record/validate resulting analysis." },
  { id: "MODULE_SURGERY", name: "Surgical Block", desc: "Operating room scheduling, pre-op checklists, and post-op routing." },
  { id: "MODULE_RADIOLOGY", name: "Radiology Unit", desc: "Integration endpoints for PACs, imaging requests, and diagnostic reports." },
  { id: "MODULE_BILLING", name: "Financial & PMSI", desc: "Stay coding, DIM authorization workflows, and accounting integration." },
  { id: "MODULE_PLANNING", name: "RH & Staff Planning", desc: "Schedule generation, shift assignments, department trees, and absence logs." }
];

export default function SettingsPage() {
  const { currentUser, setTenantId, activeModules, setActiveModules } = useAppStore();
    ...
    <div className="flex flex-col h-full space-y-4 max-w-5xl mx-auto w-full pb-8">
      <div className="flex items-center justify-between shrink-0 bg-white p-4 rounded border border-slate-200 shadow-sm mt-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Configure your environment, user profile, and active clinical modules.</p>
        </div>
        ...
        <Tabs defaultValue="profile" orientation="vertical" className="flex-1 w-full flex flex-col md:flex-row gap-8">
           <TabsList className="flex flex-col justify-start h-auto bg-transparent items-stretch space-y-1 md:w-64 shrink-0 p-0">
             <TabsTrigger value="profile" className="justify-start px-4 py-2.5 text-sm rounded-md text-slate-600 transition-all data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 data-[state=active]:font-semibold data-[state=active]:shadow-none hover:bg-slate-100">
               <User className="h-4 w-4 mr-3" /> User Profile
             </TabsTrigger>
             ...
             {isSysAdmin && (
               <>
                 <TabsTrigger value="users" className="justify-start px-4 py-2.5 text-sm rounded-md text-slate-600 transition-all data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 data-[state=active]:font-semibold data-[state=active]:shadow-none hover:bg-slate-100">
                   <Users className="h-4 w-4 mr-3" /> Users & Roles
                 </TabsTrigger>
                 <TabsTrigger value="organization" className="justify-start px-4 py-2.5 text-sm rounded-md text-slate-600 transition-all data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 data-[state=active]:font-semibold data-[state=active]:shadow-none hover:bg-slate-100">
                   <Building className="h-4 w-4 mr-3" /> Organization
                 </TabsTrigger>
                 <TabsTrigger value="modules" className="justify-start px-4 py-2.5 text-sm rounded-md text-slate-600 transition-all data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 data-[state=active]:font-semibold data-[state=active]:shadow-none hover:bg-slate-100">
                   <LayoutTemplate className="h-4 w-4 mr-3" /> Clinical Modules
                 </TabsTrigger>
                 ...
                 <TabsTrigger value="security" className="justify-start px-4 py-2.5 text-sm rounded-md text-slate-600 transition-all data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 data-[state=active]:font-semibold data-[state=active]:shadow-none hover:bg-slate-100">
                   <ShieldCheck className="h-4 w-4 mr-3" /> Security & Access
                 </TabsTrigger>
                 <TabsTrigger value="integrations" className="justify-start px-4 py-2.5 text-sm rounded-md text-slate-600 transition-all data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 data-[state=active]:font-semibold data-[state=active]:shadow-none hover:bg-slate-100">
                   <Link2 className="h-4 w-4 mr-3" /> Integrations (HL7/API)
                 </TabsTrigger>
               </>
             )}
           </TabsList>
              ...
              <TabsContent value="profile" className="m-0 mt-0 focus-visible:outline-none">
                 <div className="bg-white rounded border border-slate-200 shadow-sm p-6 space-y-6">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Update your account details and directory role.</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                       ...
                       <div>
                         <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">System Role</label>
                         <Input defaultValue={currentUser?.role} disabled className="text-sm h-10 bg-slate-50 text-slate-500 font-mono" />
                       </div>
                       <div>
                         <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">Language Preference</label>
              ...
              {isSysAdmin && (
                <>
                  ...
                  <TabsContent value="organization" className="m-0 mt-0 focus-visible:outline-none">
                     <div className="bg-white rounded border border-slate-200 shadow-sm p-6 space-y-6">
                    <div>
                      <h2 className="text-lg font-bold text-slate-900">Facility Configuration</h2>
                      <p className="text-xs text-slate-500">Manage hospital details for formal reports and headers.</p>
                    </div>
                    <div className="h-px bg-slate-100 w-full" />
                    <div className="space-y-6">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="md:col-span-2">
                            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">Facility Name</label>
                          ...
                          <div>
                            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">Contact Phone Number</label>
                            <Input defaultValue="+33 1 00 00 00 00" type="tel" className="text-sm h-10 transition-all focus-visible:ring-blue-500 bg-white" />
                          </div>
                          <div className="md:col-span-2">
                            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">Physical Address</label>
                 ...
                 <div className="bg-white rounded border border-slate-200 shadow-sm p-0 overflow-hidden">
                    <div className="p-6 pb-4 border-b border-slate-100 bg-slate-50/50">
                      <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                        <LayoutTemplate className="h-5 w-5 text-blue-600" /> System Module Configuration
                      </h2>
                      <p className="text-xs text-slate-500 mt-1">Activate or deactivate operational boundaries across the tenant. This immediately updates the side-navigation availability.</p>
                    </div>
                               ...
                               <div className="flex-1">
                                  <div className="flex justify-between items-start">
                                     <h3 className={cn("text-sm font-bold", isActive ? "text-slate-900" : "text-slate-500")}>
                                       {mod.name} 
                                       {mod.required && <span className="ml-2 text-[9px] uppercase tracking-wider bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-semibold border border-slate-200">Required core</span>}
                                       {isActive && !mod.required && <span className="ml-2 text-[9px] uppercase tracking-wider bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded font-semibold border border-blue-200">Active</span>}
                                     </h3>
