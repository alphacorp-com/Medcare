  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-slate-500">
        ...
        <h2 className="text-xl font-bold text-slate-700">Account Not Found</h2>
        <p className="text-sm mt-2">The user you are looking for does not exist or has been removed.</p>
        <Button variant="outline" className="mt-6" onClick={() => router.push('/settings')}>Return to Settings</Button>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full space-y-6 max-w-7xl mx-auto w-full pb-12 pt-4 px-4 sm:px-6">
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-6 md:items-center items-start">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-2 text-sm text-slate-500">
            <span className="flex items-center gap-1.5"><Building2 className="w-4 h-4 text-slate-400" /> {user.role}</span>
            <span className="flex items-center gap-1.5"><Mail className="w-4 h-4 text-slate-400" /> {user.email}</span>
            <span className="flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-slate-400" /> {user.modules.length} Modules Authorized</span>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 pb-2 border-b border-slate-200 mt-8">
        ...
        <div>
          <h2 className="text-lg font-bold text-slate-900">Audit History Log</h2>
          <p className="text-xs text-slate-500">Chronological history of interactions and system changes by this user.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        {activities.length === 0 ? (
          <div className="p-12 text-center flex flex-col items-center justify-center text-slate-500">
            ...
            <p className="text-xs mt-1 text-slate-400">This user hasn't performed any logged actions in the system yet.</p>
          </div>
        ) : (
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow>
                ...
                <TableHead>Detailed Payload</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {activities.map((act) => (
