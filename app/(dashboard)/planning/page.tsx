    ...
    return (
      <div className="flex h-full items-center justify-center">
         <div className="text-center p-8 bg-white border border-slate-200 rounded-lg max-w-md shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-500">The RH & Planning module is not active for your role, or has been disabled across the environment.</p>
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      <div className="flex items-center justify-between shrink-0 bg-white p-4 rounded border border-slate-200 shadow-sm">
        <div>
          <h1 className="text-lg font-bold text-slate-800">RH & Staff Planning</h1>
          <p className="text-xs text-slate-500 mt-1">Manage departmental schedules, shifts, and staff absences.</p>
        </div>
        <div className="flex bg-slate-100 p-1 rounded-md">
          ...
          >
            <CalendarDays className="h-3.5 w-3.5 mr-2" /> Shift Rosters
          </button>
          <button 
          ...
          >
            <Building2 className="h-3.5 w-3.5 mr-2" /> Departments
          </button>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-100 text-[10px] text-slate-500 uppercase font-bold sticky top-0 z-10 shadow-sm">
                  <th className="px-4 py-3 border-b border-r border-slate-200 w-48 bg-slate-100">Staff Member</th>
                  {weekDays.map((date, i) => (
                 ...
                 {departments.map(dept => (
                    <div key={dept.id} className="border border-slate-200 rounded p-4 shadow-sm hover:shadow hover:border-blue-300 transition-all cursor-pointer">
                       ...
                       <div className="flex items-center gap-2 mb-2">
                          ...
                          <div className="text-xs text-slate-600"><span className="text-slate-400">Head:</span> {dept.head}</div>
                       </div>
        ...
        <DialogContent className="sm:max-w-sm p-0 overflow-hidden flex flex-col gap-0 border-0 outline-none">
           <DialogHeader className="p-4 border-b border-slate-200 flex flex-row justify-between items-center bg-slate-50 space-y-0">
              <DialogTitle className="text-sm font-bold text-slate-800 flex items-center gap-2">
                 <Clock className="h-4 w-4 text-blue-600" /> Schedule Shift
              </DialogTitle>
           </DialogHeader>
           <div className="p-4 overflow-y-auto space-y-4 max-h-[80vh]">
              ...
              <div className="grid grid-cols-2 gap-3">
                 ...
                 <div>
                     <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Shift Type</label>
           ...
           <div className="p-4 border-t border-slate-200 bg-slate-50 flex justify-end gap-2 shrink-0">
              <Button variant="outline" size="sm" onClick={() => { setIsNewShiftOpen(false); setSelectedStaff(null); }} className="text-xs h-8">Cancel</Button>
              <Button size="sm" onClick={() => { setIsNewShiftOpen(false); setSelectedStaff(null); }} className="bg-blue-600 hover:bg-blue-700 text-white text-xs h-8">Assign Shift</Button>
           </div>
        </DialogContent>
      </Dialog>
